...
namespace ConsoleApp1
{
    class AABB
    {
        ...
        public AABB(Vector2 min2, Vector2 max2)
        {
            this.min2 = min2;
            this.max2 = max2;
        }
        ...
        {
            return new Vector2(Math.Abs(max2.x - min2.x) * 0.5f,
                               Math.Abs(max2.y - min2.y) * 0.5f);
        }
        ...
        {
            // test for not overlapped as it exits faster
            return !(p.x < min2.x || p.y < min2.y || p.x > max2.x || p.y > max2.y);
        }

        public bool Overlaps(AABB other)
        {
            // test for not overlapped as it exits faster
            return !(max2.x < other.min2.x || max2.y < other.min2.y || min2.x > other.max2.x || min2.y > other.max2.y);
        }
